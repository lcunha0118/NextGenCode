...
CAMELS_516["N_nexus"]=0
print(len(CAMELS_516))
for i in range(0,len(CAMELS_516)):
    Folder=CAMELS_516['Folder_CAMELS'].iloc[i]
    Nexus_file=Hydrofabrics_folder+"/"+Folder+"/spatial/nexus_data.geojson"
    nexus=gpd.read_file(Nexus_file)
    CAMELS_516.at[CAMELS_516.index[i],"N_nexus"]=len(nexus)
    print(len(nexus))
...
Hydrofabric_RFC = Hydrofabric_RFC[['Folder_CAMELS','NCat','N_nexus', 'SA_analysis','NOAH_CFE', 'area_sqkm', 'p_mean', 'pet_mean',
...
Hydrofabric_RFC_physio=Hydrofabric_RFC_physio[['Folder_CAMELS','NCat','N_nexus', 'SA_analysis','NOAH_CFE', 'area_sqkm', 'p_mean', 'pet_mean',
       'p_seasonality', 'frac_snow', 'aridity', 'high_prec_freq', 'high_prec_dur',
       'high_prec_timing', 'low_prec_freq', 'low_prec_dur', 'low_prec_timing', 'geometry',
...
Hydrofabric_RFC_physio_eco=Hydrofabric_RFC_physio_eco[['Folder_CAMELS','NCat','N_nexus', 'SA_analysis','NOAH_CFE', 'area_sqkm', 'p_mean', 'pet_mean',
       'p_seasonality', 'frac_snow', 'aridity', 'high_prec_freq', 'high_prec_dur',
       'high_prec_timing', 'low_prec_freq', 'low_prec_dur', 'low_prec_timing', 'geometry',
...
Hydrofabric_RFC_physio_eco_clima=Hydrofabric_RFC_physio_eco_clima[['Folder_CAMELS','NCat','N_nexus', 'SA_analysis','NOAH_CFE', 'area_sqkm', 'p_mean', 'pet_mean',
       'p_seasonality', 'frac_snow', 'aridity', 'high_prec_freq', 'high_prec_dur',
       'high_prec_timing', 'low_prec_freq', 'low_prec_dur', 'low_prec_timing', 'geometry',
...
Hydrofabric_RFC_physio_eco_clima_hlr=Hydrofabric_RFC_physio_eco_clima_hlr[['Folder_CAMELS','NCat','N_nexus', 'SA_analysis','NOAH_CFE', 'area_sqkm', 'p_mean', 'pet_mean',
       'p_seasonality', 'frac_snow', 'aridity', 'high_prec_freq', 'high_prec_dur',
       'high_prec_timing', 'low_prec_freq', 'low_prec_dur', 'low_prec_timing', 'geometry',
...
for i in HLR_unique:
    ...
    selected_sort.at[selected_sort.index[1],'Priority']=3
...
RFC_unique=Hydrofabric_RFC_physio_eco_clima_hlr['RFC'].unique()

for i in RFC_unique:
    
    selected=Selected_calibration[(Selected_calibration['RFC']==i)]
    selected2=Hydrofabric_RFC_physio_eco_clima_hlr[(Hydrofabric_RFC_physio_eco_clima_hlr['RFC']==i) & (Hydrofabric_RFC_physio_eco_clima_hlr['Available_data']>80) & (Hydrofabric_RFC_physio_eco_clima_hlr['NOAH_CFE']>0)]
    if(len(selected)==0):
        selected_sort=selected2.sort_values(by='NCat')
        selected_sort['Priority']=-9
        selected_sort.at[selected_sort.index[0],'Priority']=2
        selected_sort.at[selected_sort.index[1],'Priority']=4
        Selected_calibration=pd.concat([Selected_calibration,selected_sort[0:2]])
    if(len(selected)==1):
        selected_sort=selected2.sort_values(by='NCat')
        selected_sort['Priority']=-9
        selected_sort.at[selected_sort.index[0],'Priority']=4
        Selected_calibration=pd.concat([Selected_calibration,selected_sort[0:1]])  

Selected_calibration.index.name="hru_id_CAMELS"
Selected_calibration.to_csv(Hydrofabrics_folder+"Select_calibration_HLR_RFC.csv")
Selected_calibration.to_file(Hydrofabrics_folder +'Select_calibration_HLR_RFC.shp')