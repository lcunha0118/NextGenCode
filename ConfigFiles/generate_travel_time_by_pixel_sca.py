...
def ContantVelocityMS(namestr,taudem_folder,hydrofabrics_folder,channel,overland,gully):
    ...
    source=taudem_folder+namestr+".tif"
    rds = gdal.Open(source, GA_ReadOnly)
    ...
    print("ProjSys source " + ProjSys)

    
    ad8_file=taudem_folder+namestr+"sca.tif"
    rds = gdal.Open(ad8_file, GA_ReadOnly)
    assert rds, "Could not open " + ad8_file
    rb = rds.GetRasterBand(1) 
    print("ProjSys" + ProjSys)
    ad8_array = rb.ReadAsArray()
    
    src_file=taudem_folder+namestr+"hf.tif"
    ...
    min_ad8=ad8_array.min()
    print("min_ad8 " + str(min_ad8))
    inv_vel_matrix=np.zeros((nrow,ncol))+overland
    inv_vel_matrix[ad8_array>90]=gully
