...
def generate_twi_per_basin(namestr,catchments, twi_raster,slope_raster, dist_to_outlet_raster, NWM_param_file,outputfolder_twi,
    ...
    if(output_flag==1) & (os.path.isfile(NWM_param_file)): 
        ...
        soil_params['dksat_soil_layers_stag=1_Time=1']= soil_params['dksat_soil_layers_stag=1'].fillna(1)
    else:
    ...
    while feat is not None:
        ...
        if not src_array is None:
            ...
            if(Check>50):    
                ...
                print("all_valid_values_in_basin " + str( all_valid_values_in_basin))
                print("Check  "+str(Check))
                print("len(np.unique(sorted_array))  "+str(len(np.unique(sorted_array))))
                if(len(np.unique(sorted_array))<=0):
                    print(cat)
                    print(sorted_array)
                    print(masked)
                    print(maskedArray)
...
# generate_twi_per_basin(namestr,catchments, twi_raster, slope_raster, dist_to_outlet_raster, NWM_param_file, outputfolder_twi,
