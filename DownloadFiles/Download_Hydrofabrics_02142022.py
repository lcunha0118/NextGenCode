...
for i in range (0,len(CAMELS_516)):    
    ...
    str_sub="rm " +outfolder+"flowpath_data_4269.json"  
    out=subprocess.call(str_sub,shell=True)  

    str_sub="ogr2ogr -f \"GeoJSON\" "+outfolder+"flowpath_data_4269.json " +outfolder+"flowpath_data.geojson -s_srs EPSG:5070 -t_srs EPSG:4269"
...
for i in range (0,len(CAMELS_516)):
    ...
    outfolder=Hyd_folder+"/"+Folder+"/spatial/"
    Hydrofabrics=outfolder+"catchment_data.geojson"
    # str_sub="ogr2ogr -f \"GeoJSON\" "+outfolder+"flowpath_data.geojson " +outfolder+"hydrofabric.gpkg" + " flowpaths"    
    # out=subprocess.call(str_sub,shell=True)  
    # str_sub="ogr2ogr -f \"GeoJSON\" "+outfolder+"catchment_data.geojson " +outfolder+"hydrofabric.gpkg" + " catchments"    
    # out=subprocess.call(str_sub,shell=True)   
    str_sub="ogr2ogr -f \"GeoJSON\" "+outfolder+"nexus_data.geojson " +outfolder+"hydrofabric.gpkg" + " nexus"    
    out=subprocess.call(str_sub,shell=True) 
...
for i in range (0,len(CAMELS_516)):
    ...
    str_sub="sed -i \'s/ID/id/g\' " +outfolder+"*"
    out=subprocess.call(str_sub,shell=True) 

    str_sub="sed -i \'s/toID/toid/g\' " +outfolder+"*"
    out=subprocess.call(str_sub,shell=True)     
