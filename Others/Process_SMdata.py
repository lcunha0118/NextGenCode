...
Directory="/media/west/Expansion/Projects/SM/scan/"
all_results=glob.glob(Directory+"*.csv")
columns=['stationID','Lat','Lon','Filename']
SCAN=pd.DataFrame(columns=columns)
for i in range(0,len(all_results)):
    Temp=pd.read_csv(all_results[i])
    data =[Temp['Station ID'].iloc[0],Temp['lat'].iloc[0],Temp['lon'].iloc[0],all_results[i]]
    Temp_df=pd.DataFrame(data=[data],columns=columns)
    SCAN=pd.concat([SCAN,Temp_df])
#SCAN=pd.read_csv("/media/west/Expansion/Projects/SM/nsmn_SCAN_nwm_collocations.txt", delimiter=r"\s+")
SCAN_gf = gpd.GeoDataFrame(
    SCAN, geometry=gpd.points_from_xy(SCAN.Lon, SCAN.Lat))
...
Directory="/media/west/Expansion/Projects/SM/uscrn/"
all_results=glob.glob(Directory+"CRNH*.csv")
columns=['stationID','Lat','Lon','Filename']
USCRN=pd.DataFrame(columns=columns)
for i in range(0,len(all_results)):
    Temp=pd.read_csv(all_results[i])
    data =[Temp['stationID'].iloc[0],Temp['Lat'].iloc[0],Temp['Lon'].iloc[0],all_results[i]]
    Temp_df=pd.DataFrame(data=[data],columns=columns)
    USCRN=pd.concat([USCRN,Temp_df])
...
USCRN_gf = gpd.GeoDataFrame(
    USCRN, geometry=gpd.points_from_xy(USCRN.Lon, USCRN.Lat))
