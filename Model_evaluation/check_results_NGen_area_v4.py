...
def plot_Results_v2(Model,Output_config,Total,CAMEL_Daymet_Acum,NWM_21,Title_str,output_figure):        
    ...
    if("CFE" in Model) |  ("Topmodel" in Model):        
        ...
        if(len(NWM_21)>0): ax_compare[0].plot (NWM_21_Acum.index,NWM_21_Acum['flow_cms'], label = 'NWM 2.1 runoff', color= 'forestgreen',linewidth=1.5) #Plot obs data on comparison figure
        ...
        if(len(NWM_21)>0): ax_compare[1].plot(NWM_21.index,NWM_21['flow_cms'], label = 'NWM 2.1',linewidth=1.5, color= 'forestgreen',alpha=0.6) #plot the rainfall from the mixed AORC/other precip array used to generate the equations
...
def plot_Results_v3(Model,Output_config,Total,CAMEL_Daymet_Acum,NWM_21,Title_str,output_figure):        
    ...
    if("CFE" in Model) |  ("Topmodel" in Model):        
        ...
        if(len(NWM_21)>0): ax_compare[0].plot (NWM_21_Acum.index,NWM_21_Acum['flow_cms'], label = 'NWM 2.1 runoff', color= 'forestgreen',linewidth=2) #Plot obs data on comparison figure
        ax_compare[0].plot (Total.index,Total_Acum['Obs_Q'], label = 'Obs runoff', color= 'blue',linewidth=1) #Plot obs data on comparison figure
...
def plot_SM(Model,Output_config,Total,Obs,Cat,Title_str,output_figure):        
    ...
    if("CFE" in Model) |  ("Topmodel" in Model):        
        ...
        for i in range(0,len(Obs_F.columns)):
            if(Obs_F.columns[i]=='sm_20cm') | (Obs_F.columns[i]=='sm_50cm') | (Obs_F.columns[i]=='sm_100cm'):
                #print (To_plot.iloc[i].output_var_names)
                ax_compare[5].plot (Obs_F.index,Obs_F[Obs_F.columns[i]], label = Obs_F.columns[i],linewidth=0.75) #Plot obs data on comparison figure
                
        ax_compare[5].legend(loc='upper left')
        #ax_compare[2].set_xlabel ('Date'); 
        ax_compare[5].set_ylabel('Observed Storage (%)' )
        #ax_compare[0].set_title(Title_str)
...
def plot_SNOW(Model,Output_config,Total,SWE,Cat,Title_str,output_figure):        
    
   
    Total_Acum=Total.cumsum(axis= 0)
    CAMEL_Daymet_Acum=CAMEL_Daymet.cumsum(axis= 0)
    NWM_21_Acum=NWM_21.cumsum(axis= 0)
    SWE=SWE[(SWE.index>=Total_Acum.index.min()) & (SWE.index<=Total_Acum.index.max())]
    CB_color_cycle = ['#377eb8', '#ff7f00', '#4daf4a',
                  '#f781bf', '#a65628', '#984ea3',
                  '#999999', '#e41a1c', '#dede00']
    ...
    if("CFE" in Model) |  ("Topmodel" in Model):        
        ...
        #ax_compare[5].plot (Total_daily.index,Total_daily[Output_config.loc['Surface water flux'].output_var_names], label = 'Accum surface water flux', color= 'black') #Plot obs data on comparison figure
        ax_compare[5].plot (SWE.index,SWE, label = 'SWE',linewidth=0.75) #Plot obs data on comparison figure
                
        ax_compare[5].legend(loc='upper left')
        #ax_compare[2].set_xlabel ('Date'); 
        ax_compare[5].set_ylabel('SWE/Surface water flux (m)' )
        #ax_compare[0].set_title(Title_str)
...
def plot_PET(Model,Output_config,PETts,ETts,output_figure):        
    ...
    if("CFE" in Model) |  ("Topmodel" in Model):        
        num_subplots = 1 #If plotting cumulative precipitation/soil moisture, use 3 subplots (flow, cumulative rain, soil moisture)
        plt.rcParams["figure.figsize"] = (8,16/6)
        fig, ax_compare = plt.subplots(num_subplots, 1) #Create the plot for this zone
        #ax_compare[0].set_title('Cumulative Precipitation')
        ax_compare.plot (PETts.index,PETts[Output_config.loc['POTENTIAL_ET'].output_var_names], label = Model, color= 'orange',linewidth=1) #Plot obs data on comparison figure
        #ax_compare[0].plot (NWM_21_Acum.index,NWM_21_Acum['flow_cms'], label = 'NWM 2.1 runoff', color= 'forestgreen',linewidth=2) #Plot obs data on comparison figure
        ax_compare.plot (ETts.index,ETts[Output_config.loc['ACTUAL_ET'].output_var_names], label = Model, color= 'blue',linewidth=1) #Plot obs data on comparison figure

        ax_compare.set_ylabel('PET/ET (m/h)')
...
Output_ngen=["Output_ngen_08302022_4"]
...
#Models=["NOAH_CFE"]
#Models=["NOAH_Topmodel"]
Models=["PET_1_CFE","NOAH_CFE","PET_2_CFE","PET_3_CFE","PET_4_CFE","PET_5_CFE"]
#Models=["PET_1_CFE"]
...
#CAMELS_516=CAMELS_516[CAMELS_516.index=="12035000"]
NAN_in_results=[]
for i in range (0,len(CAMELS_516)):  
    ...
    if(hru_id in SCAN_join.gauge_id.values): 
        ...
        Special_case=Special_case+1
    if(int(hru_id) in SNOTEL_join.gauge_id.values): 
        SNOTEL_flag=1
        cat_Special.append(['SNOTEL',SNOTEL_join[SNOTEL_join.gauge_id==int(hru_id)].id.values[0],SNOTEL_join[SNOTEL_join.gauge_id==int(hru_id)].nsite.values[0]-1])
        Special_case=Special_case+1
        ...
        for z in range(0,len(Output_ngen)):
            ...
            if ("PET" in Models[j]) and (CAMELS_516.iloc[i]['RunPET']==0):
                ...
                if(len(all_results)<len(zones)) | (flag_empty==1) | (flag_wrong==1):
                    ...
                    if(os.path.isfile(Total_file)) & (Special_case==0) & (Rerun == 0):
                        ...
                        for index,row in zones.iterrows():
                            ...
                            Obs_RR_mmh_Temp=Obs_RR_mmh_Temp['RAINRATE'].to_frame()*3.6
                        ...
                        Rainfall_from_forcing_mmh_ave=Rainfall_from_forcing_mmh_ave/sum(zones.area_sqkm)
                        if(Total_out.index.max()<max_date_plot):
                            ...
                            Obs_q_mh = Obs_q_mh.groupby(level=0).first()
                            ...
                            if(Generate_plots_flag==1): 
                                Generate_PET_plot(Models[j],Output_config,PETts,ETts,output_figure)
                                output_figure= Results+"PET_TS"+min_date_plot.strftime ('%Y-%m')+"_"+max_date_plot.strftime ('%Y-%m')+".png"
                                plot_PET(Models[j],Output_config,PETts,ETts,output_figure)
                            ...
                            if(Special_case>0):
                                ...
                                if(cat_Special[0][0]=='SNOTEL'):   
                                    ...
                                    swe=nc_SNOTEL['snotel_swe_pnnl'][:,cat_Special[0][2]]/1000.
                                    ...
                                    output_figure= Results_SM+hru_id+Models[j]+"SNOTEL.png"
                                    plot_SNOW(Models[j],Output_config,Total,SWE_df,Cat_out,Title_str,output_figure)
                            ...
                            date_max=Total.Obs_Q.sort_values(ascending=False).index[0]
                            min_date_plot1=date_max-timedelta(days=2)
                            max_date_plot1=date_max+timedelta(days=2)
                            Title_str=hru_id+ " - " +Models[j]+' - ' + min_date_plot1.strftime ('%Y-%m-%d')+' through '+ max_date_plot1.strftime ('%Y-%m-%d')
                            Total_temp=Total[(Total.index>=min_date_plot1) & (Total.index<max_date_plot1)] 
                            CAMEL_Daymet_temp=CAMEL_Daymet[(CAMEL_Daymet.index>=min_date_plot1) & (CAMEL_Daymet.index<max_date_plot1)] 
                            if(os.path.isfile(NWM_21_file)): 
                            ...
                            if(Generate_plots_flag==1): [Nash,NNash,mean_bias,mean_absolute_error,RMSE,NRMSE]=Generate_Scatter_plot2(Models[j],Output_config,Total,CAMEL_Daymet,NWM_21,Title_str,output_figure)
...
NAN_in_resultsDf=pd.DataFrame(data=NAN_in_results,columns=['hru_id','Model','catstr','len(Cat_out.dropna())','len(Cat_out)'])