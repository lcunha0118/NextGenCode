...
def plot_Results(Model,Total_out_plot,Obs_q_mh_plot,Obs_RR_mh_ave_plot,Title_str,output_figure):        
    ...
    if("CFE" in Model) |  ("Topmodel" in Model):
        ...
        if("CFE" in Model):    
            ...
            ax_compare[2].plot (Total_out_acum.index,Total_out_acum[q_var[j]], label = 'Total runoff (m)', color= 'r') #Plot obs data on comparison figure
            ax_compare[2].plot (Total_out_acum.index,Total_out_acum['land_surface_water__domain_evaporation_volume_flux'], label = 'evaporation_volume_flux', color= 'y') #Plot obs data on comparison figure
            ax_compare[2].plot (Total_out_acum.index,Total_out_acum['land_surface_water__baseflow_volume_flux'], label = 'baseflow_volume_flux', color= 'm') #Plot obs data on comparison figure
            ax_compare[2].plot (Total_out_acum.index,Total_out_acum['land_surface_water__domain_overland_flow_volume_flux'], label = 'overland_volume_flux', color= 'b') #Plot obs data on comparison figure
            ax_compare[2].legend(loc='upper left')
            ax_compare[2].set_xlabel ('Date'); ax_compare[2].set_ylabel('Runoff Components (m)' )
...
q_var=["Q_OUT","Qout",""]
rain_var=["RAIN_RATE","atmosphere_water__liquid_equivalent_precipitation_rate_out",""]
...
for i in range (0,len(CAMELS_516)):   
    for j in range (0,len(Models)):   
        ...
        min_date_plot=datetime(2008,5,1,0,0)
