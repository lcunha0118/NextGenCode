...
def plot_twi_CAMELS(hru_id,IDs,outputfolder_twi,outputfolder_summary,filename,xlim):
    plt.figure()
    ncolsAr=[];catAr=[]
    NoFiles=[]
    TWIList=[]
    NoFiles=pd.DataFrame(columns=(["hru_id","cat"]))
    for cat in IDs:  
        if(isinstance(cat, float)): cat=str(int(cat))
        DatFile=os.path.join(outputfolder_twi,"cat-"+str(cat),"subcat_cat-"+str(cat)+".dat")
        catAr.append(cat)
        ...
        if os.path.exists(DatFile): 
            ...
            NoFiles=pd.concat([NoFiles,pd.DataFrame([(hru_id,cat)],columns=(["hru_id","cat"]))])
            ncolsAr.append(-9)
    ...
    return NoFiles,TWIList
def plot_width_function_CAMELS(IDs,outputfolder_twi,outputfolder_summary,filename,xlim):
# Read width function
...
def plot_giuh_CAMELS(IDs,outputfolder_giuh,outputfolder_summary,filename,xlim):
